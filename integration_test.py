...
class AriadneIntegrationTest:
    ...
    def test_full_pipeline(self):
        ...
        try:
            ...
            with patch("api_recap.bedrock") as mock_bedrock, patch(
                "api_recap.store_recap"
            ) as mock_store:

                # Mock Bedrock response
                mock_response = {"body": MagicMock()}
                mock_response["body"].read.return_value.decode.return_value = (
                    json.dumps(
                        {
                            "blocks": [
                                {
                                    "type": "text",
                                    "content": "User: I need a function to calculate fibonacci numbers",
                                },
                                {
                                    "type": "code",
                                ...
                                {
                                    "type": "text",
                                    "content": "User: That looks good but won't it be slow for large numbers?",
                                },
                                {
                                    "type": "code",
                                    "content": "def fibonacci(n):\n    if n <= 1:\n        return n\n    a, b = 0, 1\n    for i in range(2, n + 1):\n        a, b = b, a + b\n    return b",
                                ...
                                {
                                    "type": "text",
                                    "content": "User: Perfect! This is my final version.",
                                },
                            ]
                        }
                    )
                ...
                if response.status_code == 200:
                    ...
                    for field in required_fields:
                        if field not in data:
                            self.log(
                                f"Missing field in response: {field}", is_error=True
                            )
                        else:
            ...
            try:
                ...
                if test_case["should_succeed"]:
                    if response.status_code == 200:
                        ...
                        self.log(
                            f"❌ Filter test '{test_case['name']}' should have succeeded but got {response.status_code}",
                            is_error=True,
                        )
                else:
                    if response.status_code != 200:
                        self.log(
                            f"✅ Filter test '{test_case['name']}' correctly rejected"
                        )
                    else:
                        self.log(
                            f"❌ Filter test '{test_case['name']}' should have been rejected",
                            is_error=True,
                        )
