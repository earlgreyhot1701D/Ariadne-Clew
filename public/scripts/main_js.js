...
function buildHumanFromStructured(structured) {
  ...
  parts.push(
    '<h3>Summary</h3>',
    sum ? `<p>${sum}</p>` : '<p class="subtle">No summary.</p>'
  );
...
function displayResults(payloadLike) {
  ...
  const structured =
    payload.structured_data || payload.raw_json || payload || {};
  let html = '';
  try {
    html = buildHumanFromStructured(structured);
  } catch (e) {
    console.warn(
      '[AC] buildHumanFromStructured failed, falling back to backend HTML',
      e
    );
...
async function handleSubmit(e) {
  ...
  try {
    ...
    if (/Failed to fetch/i.test(m))
      msg += 'Connection failed. Is bridge_server.py running?';
    else if (/HTTP 5\d\d/.test(m))
      msg += 'Server error. Check AgentCore connection.';
    else msg += m;
    setStatus(msg, true);
  } finally {
    const submitBtn = document.getElementById('submit-btn');
...
function handleCopy() {
  ...
  if (!jsonPanel) return;
  navigator.clipboard
    .writeText(jsonPanel.textContent)
...
function handleExport() {
  ...
  const markdown = `# Ariadne Clew Recap\n\n${
    ...
    resolvePayload(currentRecapData).structured_data || currentRecapData,
    null,
    2
  )}\n\`\`\``;
